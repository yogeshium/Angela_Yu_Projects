...
app.put("/jokes/:id",(req,res)=>{
    ...
    const idx = jokes.findIndex((joke)=> parseInt(joke.id)===parseInt(req.params.id));
...
app.patch("/jokes/:id",(req,res)=>{
  ...
  const idx = jokes.findIndex((joke)=> parseInt(joke.id)===parseInt(req.params.id));
  const existingJoke = jokes[idx];
  const editedJoke = {
    id: existingJoke.id,
    jokeText: req.body.text || existingJoke.jokeText,
    jokeType: req.body.type || existingJoke.jokeType,
  } ;
  jokes[idx]=editedJoke;
  res.json(editedJoke);
});
...
app.delete("/jokes/:id", (req,res)=>{
  ...
  const idx = jokes.findIndex((joke)=> parseInt(joke.id)===parseInt(id));
  if(idx>-1)
  {
    jokes.splice(idx,1);
    res.sendStatus(200);
